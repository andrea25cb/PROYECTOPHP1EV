
<table class="menu" style="
    width: 100%;
    text-align:center;
    background: #ccffcc;
    align-items: center;
    font-size:20px;
    ">
<tr>
    <td><a href='../vista/listarTareas.blade.php'><button class="btn text-left" style="color: #ccffcc; background:green;"> MENU</button></a></td>
    <td><a href='tareasPendientesAdmin.php'><button class="btn text-left" style="color: #ccffcc; background:green;"> VER TAREAS PENDIENTES</button></a></td>
    <td><a href='../vista/listarUsuarios.php'><button class="btn text-left" style="color: #ccffcc; background:green;"> LISTAR OPERARIOS</button></a></td>
    <td><a href="../cerrar_sesion.php"><button class="btn btn-danger text-left"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span> CERRAR SESION</button></a></td>

  </tr>    
</table>